...
// N1, N2를 차례로 입력받는다. 하나라도 잘못되면 0을 반환. (N1, N2는 홀수여야 함)
int INPUT_SIZE(void)
{
    printf("N1: ");
    if (inputInt() == 0)
        return 0;
    if (ipValue % 2 == 0)   // 짝수면 안 됨
    {
        printf("홀수만 입력할 수 있습니다.\n");
        return 0;
    }
    N1 = ipValue;
...
// 가운데에서 1부터 시작해 마름모(다이아몬드) 모양으로 바깥으로 돌며 채운다.
void SET_Arr(void)
{
    int value = 1;
    int row = N1 / 2;   // 가운데 행
    ...
    pArr[row][col] = value;   // 중앙 = 1
    value = value + 1;

    // k = 마름모 링의 크기 (1, 2, 3 ...), 모든 칸을 채울 때까지 반복
    for (int k = 1; value <= N1 * N2; k++)
    {
        // 한 칸 아래로 내려가서 새 링을 시작
        row = row + 1;
        if (row >= 0 && row < N1 && col >= 0 && col < N2)
        {
            pArr[row][col] = value;
            value = value + 1;
        }
