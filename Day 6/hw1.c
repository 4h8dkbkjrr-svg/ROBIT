...
// head 자체에는 값을 넣지 않고, head->next 부터가 실제 리스트이다.
// 빈 리스트: head -> NULL, 리스트의 첫 요소(index 0) = head->next
NODE* head = NULL;
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if      (cmd == 1)
            ...
            printf("리스트의 길이: %d\n", GET_length());
            // ㄴGET_length 는 값만 반환하므로 여기서 출력한다.
...
/*
 ...
 > 5. 자릿수가 int 로 담을 수 있는 범위를 넘으면 안내 후 0을 반환한다.
