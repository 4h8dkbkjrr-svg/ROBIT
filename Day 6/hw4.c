...
//  > 2. 한 줄을 읽었으면 1을, 더 읽을 것이 없는 입력의 끝이면 0을 반환한다.
//  > 3. 빈 줄은 line 이 빈 문자열인 채로 1을 반환한다.
//
//  > * INPUT_line 함수는 Claude의 도움을 받아 작성되었습니다.
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (INPUT_line(line) == 0)
            break;
            // ㄴ입력의 끝: stop 과 동일하게 종료한다.

        if (line[0] == '\0')   // 빈 줄이면 다시 입력받는다
            continue;
...
/*
 ...
 > 3. 끝에 '\0' 을 붙인다.
 > 4. 한 줄을 읽었으면 1을, 아무것도 못 읽고 입력이 끝났으면 0을 반환한다.
...
int INPUT_line(char* line)
{
    char ip;
    int  len   = 0;
    int  isEnd = 0;   // 더 읽을 것이 없는 입력의 끝(EOF)을 만났는지
    ...
    while (1)
    {
        if (scanf("%c", &ip) != 1)
        {
            isEnd = 1;
            break;
            // ㄴ더 읽을 글자가 없다. 본 처리는 Claude 조언에 따라 추가되었습니다.
        }
    ...
    if (isEnd == 1 && len == 0)   // 읽은 글자도 없이 입력이 끝남
        return 0;
    else
        return 1;
}
