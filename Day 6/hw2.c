...
/*
 ...
 > 6. 자릿수가 int 로 담을 수 있는 범위를 넘으면 안내 후 0을 반환한다.
 > 7. 통과하면 부호를 곱한 값을 out 에 담고 1을 반환한다.
...
int INPUT_int(int* out)
{
    ...
    while (1)
    {
        if (scanf("%c", &ip) != 1)   // 한 글자 읽기
            break;
            // ㄴ더 읽을 글자가 없는 입력의 끝(EOF). 본 처리는 Claude 조언에 따라 추가되었습니다.
        ...
        {
            int digit = (int)(ip - '0');   // 형변환으로 자릿값을 얻는다

            if (value > (2147483647 - digit) / 10)   // 한 자리 더 붙이면 int 범위를 넘는다
            {
                printf("정수 범위를 벗어났습니다.\n");
                return 0;
                // ㄴ범위 검사는 Claude 조언에 따라 추가되었습니다.
            }

            value = value * 10 + digit;   // 자릿수 누적
            hasDigit = 1;
            isFirst  = 0;
        }
